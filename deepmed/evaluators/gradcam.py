...
def gradcam(
    ...
    for class_ in preds_df[f'{target_label}_pred'].unique():
        ...
        patient_scores = \
            class_instance_df.groupby(patient_label)[f'{target_label}_pred'].agg(
                lambda x: sum(x == class_) / len(x))
            ...
            for j, (_, tile) in enumerate(tiles.iterrows()):
                ...
                if hasattr(dls.vocab, 'o2i'):
                    ...
                    try:
                        ...
                        dict_vocab = {w: i for i,
                                      w in enumerate(dls.vocab[-1])}
                        cls_dec = dict_vocab[str(class_)]
                ...
                ax.imshow(cam_map.detach().cpu(), alpha=0.6, extent=(
                    0, x[0].shape[2], x[0].shape[3], 0), interpolation='bilinear', cmap='magma')
        ...
        if not outfile.exists():
            ...
            for i, im in tile_dict.items():
                ...
                plt.axis('off')
                # cannot read svg thus atm gradcams are saved as PNG imgs
